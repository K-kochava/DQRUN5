...
NewsGenerator.get_feed((NewsGenerator.generate_feed(target,
                                CaseNormalizer.proper(feedCreator.create_feed(XMLreader.parseXML(path)),
                                                      '.', '\n'), '.txt')))
wordReader.generate_csv(target, wordReader.word_reader(CaseNormalizer.proper(feedCreator.create_feed(XMLreader.parseXML(path)),
                                                      sep1, sep2)), '.csv')
